.PHONY: build package run stop run-client run-server stop-client stop-server restart restart-server restart-client start-docker clean-dist clean nuke check-style check-client-style check-server-style check-unit-tests test dist setup-mac prepare-enteprise run-client-tests setup-run-client-tests cleanup-run-client-tests test-client build-linux internal-test-client
...
GO_LINKER_FLAGS ?= -ldflags \
				   "-X github.com/commune/platform/model.BuildNumber=$(BUILD_NUMBER)\
				    -X 'github.com/commune/platform/model.BuildDate=$(BUILD_DATE)'\
				    -X github.com/commune/platform/model.BuildHash=$(BUILD_HASH)\
				    -X github.com/commune/platform/model.BuildHashEnterprise=$(BUILD_HASH_ENTERPRISE)\
				    -X github.com/commune/platform/model.BuildEnterpriseReady=$(BUILD_ENTERPRISE_READY)"
...
ifeq ($(BUILDER_GOOS_GOARCH),"linux_amd64")
	...
	@# Package
	tar -C dist -czf $(DIST_PATH)-$(BUILD_TYPE_NAME)-communeCC-linux-amd64.tar.gz mattermost
