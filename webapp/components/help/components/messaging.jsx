...
export default class HelpMessaging extends React.Component {
    render() {
        ...
        message.push(localizeMessage('help.messaging.write', '**Write messages** using the text input box at the bottom of Commune CC. Press **ENTER** to send a message. Use **Shift+ENTER** to create a new line without sending a message.'));
        message.push(localizeMessage('help.messaging.reply', '**Reply to messages** by clicking the reply arrow next to the message text.'));
        message.push('![reply arrow](https://docs.youcommune.com/_images/replyIcon.PNG)');
        ...
        message.push('![markdown](https://docs.youcommune.com/_images/messagesTable1.PNG)');
        message.push(localizeMessage('help.messaging.emoji', '**Quickly add emoji** by typing ":", which will open an emoji autocomplete. If the existing emoji don\'t cover what you want to express, you can also create your own [Custom Emoji](http://docs.youcommune.com/help/settings/custom-emoji.html).'));
        message.push(localizeMessage('help.messaging.attach', '**Attach files** by dragging and dropping into Commune CC or clicking the attachment icon in the text input box.'));
